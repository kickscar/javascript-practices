...
## ES6 이전 모듈 지원
 모듈 작성 및 사용법을 알아보기 위해 하나의 파일로 작성된 애플리케이션을 모듈로 분리해 작성해 보자. 지금은 ES6 표준 모듈 시스템과 이전의 CommonJS 기반 모듈 지원이 혼재하여 사용되고 있는 상황이기 때문에 ES6 표쥰 모듈 시스템 이전의 모듈 지원도 이해하고 사용할 줄 알아야 한다. 우선, ES6 이전 모듈 지원의 모듈 작성 및 사용을 먼저 알아보고 ES6 표준 모듈 시스템을 적용해 본다.
...
3.	App() 함수를 호출해서 반환되는 객체를 출력한다.

이 애플리케이션을 App() 함수를 정의하는 모듈과 App()함수를 사용하는 모듈로 분리해 보자.
...
3.	하나의 함수 객체뿐만 아니라 하나의 일반 객체 그리고 기본 타입의 값들도 가능하다.  
...
2.	모듈의 확장자는 생략 가능하다.
 ...
 require() 함수는 Node에서만 지원하는 동기 방식의 IO 함수다. 사용시 주의할 것은 사용하는 파일의 윗부분에 초기화 단계에서 일괄적으로 사용해야 한다. 함수이기 때문에 로직이 실행되고 있는 코드에서 호출하는 것도 가능하다. 하지만 동기 방식으로 작동하기 때문에 require()가 완료될 때까지 코드가 멈추어 만일 서버 애플리케이션이라면 성능저하를 가져올 수 있다. 이는 Node 프로그래밍에서 require를 비롯한 동기식 작업을 애플리케이션 초기화에서 사용하는 일반적인 이유이기도 하다.
 
 다음은 값들과 함수등 다수의 값과 객체를 외부로 노출하는 모듈을 작성해 보자. 사실, 모듈 작성이 필요한 대부분 경우에는 module.exports 보다는 exports 객체를 생성하는 방식만으로 충분하다. 하지만, module.exports로만 모듈 작성이 가능한 경우가 있고 module.exports가 exports보다는 구조화된 모듈 작성에 좋다.
...
2.	외부에서는 이 속성 이름으로 값 또는 함수에 접근하면 된다.
 ...
 ex03을 보면 exports로 객체를 생성하는 것 처럼 보이지만 사실은 module.exports 객체에 exports라는 이름으로 접근하고 있는 코드다. 즉, 다음 코드가 먼저 실행되고 require() 함수의 아규먼트로 전달된 모듈의 코드가 실행되는 것으로 이해하면 모듈 작성과 사용에 큰 실수를 하지 않는다.
...
```
 ...
 간단한 이 두 줄 코드를 보면, exports는 먼저 생성된 module.exports 객체의 별칭(alias)과 같다. 그리고 require() 함수는 항상 module.exports에 대입된 객체를 반환한다. 따라서 다음에 ex03/math.js과 같은 모듈 코드의 실행은 별 문제없이 기대했던대로 모듈의 값과 함수에 접근할 수 있다. 하지만 모듈이 다음과 같이 작성되어 있다면 기대와 다른 값과 객체들이 반환되거나 반환되지 못할 것이다.
...
2. 결과적으로 module.exports와 exports를 함께 사용하면 순서와 상관없이 exports의 기대했던 것은 무시된다.
3. 이유는 맨 처음 언급한 두 줄 코드가 require() 함수 맨 앞에서 실행되기 때문이다.


## ES6 표준 모듈 시스템
 Node V14+ 부터 공식 지원하는 ES6 모듈 시스템은 이전 CommonJS 기반의 모듈 지원과 호환성이 없다. "use strict"의 존재 유무와 상관없이 무조건 strict 모드다. 따라서 CommonJS 기반의 모둘 지원으로 작성된 모듈은 ES6 표준 모듈 시스템의 방법으로 불러 올수 없으며 ES6 표준 모듈 시스템으로 작성된 모듈은 require() 함수로 불러 올 수 없다.
 그리고 ES6 모듈 시스템이 작동하기 위해서는 package.json의 type 속성이 'module'로 설정되어 있어야 한다. 보통 Node기반 개발 프로젝트 디렉토리에는 `npm init` 명령을 통해 프로젝트 매니페스트 파일(패키지 관리 파일) package.json 파일이 존재한다. 이와 관련된 더 자세한 내용은 '11장 개발 환경'의 npm을 설명하면서 자세히 다룬다. 
